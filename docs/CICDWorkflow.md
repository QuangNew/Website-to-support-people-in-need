...
| Name | Ví dụ giá trị |
|------|----------------|
| `VITE_API_URL` | `https://reliefconnect-api.azurewebsites.net` |
...
> **Lưu ý:** Nếu bạn đã có file `client/.env.production` được commit vào repo với các giá trị đúng, bước này có thể bỏ qua — workflow sẽ dùng giá trị từ `.env.production`.
...
| **Nhạy cảm — phát triển** | JWT key, DB connection string, SMTP password, Gemini API key | `appsettings.Development.json` (gitignored) |
...
**Quy tắc:**
- ✅ `VITE_API_URL=https://reliefconnect-api.azurewebsites.net` — OK (URL công khai)
