...
-- Lưu ý: PostgreSQL không cho viết UNIQUE constraint trực tiếp trên biểu thức
-- trong CREATE TABLE theo cách cũ. Cách an toàn hơn là luôn lưu cặp user theo
-- thứ tự đã chuẩn hoá: User1Id < User2Id, sau đó UNIQUE trên 2 cột này.
-- ============================================================================
CREATE TABLE IF NOT EXISTS "DirectConversations" (
  ...
  CONSTRAINT "CK_DirectConversation_NoSelf" CHECK ("User1Id" <> "User2Id"),
  CONSTRAINT "CK_DirectConversation_UserOrder" CHECK ("User1Id" < "User2Id"),
  CONSTRAINT "UQ_DirectConversation_Pair" UNIQUE ("User1Id", "User2Id")
);
...
CREATE TABLE IF NOT EXISTS "DirectMessages" (
  ...
  CONSTRAINT "CK_DirectMessage_Content_NotBlank" CHECK (char_length(btrim("Content")) > 0),
  CONSTRAINT "CK_DirectMessage_Content_MaxLen" CHECK (char_length("Content") <= 2000)
);
...
BEGIN
  ...
  IF btrim(p_user1_id) = '' OR btrim(p_user2_id) = '' THEN
    RAISE EXCEPTION 'User ids cannot be blank';
  END IF;
  ...
  -- Tìm conversation hiện tại bằng cặp id đã chuẩn hoá
  SELECT "Id" INTO v_conv_id
  FROM "DirectConversations"
  WHERE "User1Id" = v_min_id
    AND "User2Id" = v_max_id;
...
BEGIN
  IF p_conversation_id IS NULL OR p_sender_id IS NULL THEN
    RAISE EXCEPTION 'Conversation id and sender id are required';
  END IF;
  ...
  VALUES (p_conversation_id, p_sender_id, v_content, v_sent, FALSE)
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM "DirectConversations"
    WHERE "Id" = p_conversation_id
      AND ("User1Id" = p_reader_id OR "User2Id" = p_reader_id)
  ) THEN
    RAISE EXCEPTION 'User is not a participant of this conversation';
  END IF;
...
DECLARE
  v_count INT;
  v_affected_conversation_ids INT[];
BEGIN
  ...
  SELECT COALESCE(array_agg(DISTINCT "ConversationId"), ARRAY[]::INT[]), COUNT(*)::INT
  INTO v_affected_conversation_ids, v_count
  FROM deleted_rows;

  IF v_count > 0 THEN
    UPDATE "DirectConversations" dc
    SET "LastMessageAt" = latest."LastMessageAt"
    FROM (
      SELECT ids."ConversationId", MAX(dm."SentAt") AS "LastMessageAt"
      FROM unnest(v_affected_conversation_ids) AS ids("ConversationId")
      LEFT JOIN "DirectMessages" dm
        ON dm."ConversationId" = ids."ConversationId"
       AND dm."DeletedAt" IS NULL
      GROUP BY ids."ConversationId"
    ) latest
    WHERE dc."Id" = latest."ConversationId";
  END IF;

  -- Log kết quả vào SystemLogs theo schema hiện tại của repo
  INSERT INTO "SystemLogs" ("Action", "Details", "CreatedAt", "UserName")
  VALUES (
    'MessageCleanup',
...
3. **Nếu từng chạy bản SQL lỗi trước đó**: kiểm tra và xoá object dở dang trước khi chạy lại để tránh lệch schema
...
`User1Id` và `User2Id` phải luôn được lưu theo thứ tự đã chuẩn hoá (`User1Id < User2Id`) để khớp với unique constraint trong SQL migration.
...
- OnModelCreating: map unique pair trên `User1Id + User2Id` và thêm check constraint để ép thứ tự `User1Id < User2Id`
...
2. **Khi user connect lại:** Frontend gọi lại `GET /api/messages/conversations` và reload conversation đang mở để sync tin nhắn bị miss lúc offline/reconnecting.
...
Details = $"User {senderId} -> {receiverId} in conv {convId}"
...
- Gọi SQL function `cleanup_old_direct_messages()` để vừa xoá message cũ vừa đồng bộ lại `LastMessageAt`
...
// Khi reconnect: sync unread count + reload conversation đang mở
connection.onreconnected(async () => {
  const store = messageStore.getState();
  await store.loadConversations();
  if (store.activeConversationId) {
    await store.loadMessages(store.activeConversationId);
  }
});
...
2. **Reconnect sync**: Frontend gọi lại `GET /conversations` và reload conversation đang mở khi reconnect
...
5. **UNIQUE constraint**: cặp `User1Id + User2Id` đã được chuẩn hoá thứ tự và dùng `ON CONFLICT` để tránh race condition
